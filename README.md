...
* **Zenodo Archive:** https://doi.org/10.5281/zenodo.20617639
* **Target Repositories:** Apache Commons-Lang (`commons-lang`, default), Checkstyle (`checkstyle`), Apache Dubbo (`dubbo`), JUnit (`junit-framework`), and QuestDB (`questdb`). Select one via the `TARGET_REPO` parameter in Phase 0.
## 🛠️ Upstream Release Tag Mining (Included)
 
While the main Colab notebook utilizes the pre-mined datasets from Zenodo to ensure deterministic reproducibility, we have also included our **Release Tag Mining Pipeline** (`rel_tag_mining.ipynb`) in this repository. 
 
This notebook demonstrates the exact methodology used to scrape the GitHub API, filter for stable official releases, and generate the chronological targeting ledgers (`rel_hist_[REPO_NAME].json`) that fed our upstream static analysis cluster.
 
## 🧩 Pipeline Architecture
 
The replication notebook is separated into six logical phases (Phase 0 through Phase 5) to prevent data leakage and ensure mathematical rigor:
...
* **Phase 2: Matrix Engineering & Partitioning.**
  * `B.2`: Traditional Spatial 80/20 split (Baseline).
...
```
3. Open ```inflection_analysis.ipynb``` in Jupyter Notebook or JupyterLab and execute the cells sequentially. The ```BASE_DIR``` paths will automatically adapt to your local working directory.
## 📄 Citation
